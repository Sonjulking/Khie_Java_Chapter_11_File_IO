 ...
 * =>예를 든다면 프로그램은 직접 하드 디스크에 데이터를 보내지 않고 메모리 버퍼에 데이터를 보냄으로써 쓰기 속도가 향상이 된다.
 * =>버퍼는 데이터가 쌓이기만을 기다렸다가 버퍼가 꽉 차게되면 데이터를 한꺼번에 하드디스크로 보내줌으로써 출력 횟수를 줄여주게 된다.
 * =>스트림의 기능(속도)를 향상시키는 클래스
 * =>Bufferedxxx : 버퍼를 제공해 주는 보조 스트림 클래스 
 * =>버퍼(buffer) : CPU 와 IO간의 속도 차이를 보완.
 */
